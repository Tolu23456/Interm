...
// Internal version that doesn't push to undo
...
im_result_t im_buffer_insert(im_buffer_t* buf, size_t pos, const char* text, size_t len) {
    ...
    if (res == IM_OK && len > 0) {
        push_undo(buf, IM_EDIT_INSERT, pos, text, len);
    }
    return res;
}
...
im_result_t im_buffer_delete(im_buffer_t* buf, size_t pos, size_t len) {
    ...
    if (res == IM_OK) {
        push_undo(buf, IM_EDIT_DELETE, pos, deleted_text, len);
    }
    free(deleted_text);
    return res;
}
...
im_result_t im_buffer_undo(im_buffer_t* buf) {
    if (!buf->undo_stack) return IM_ERR;
    
    im_undo_record_t* rec = buf->undo_stack;
    buf->undo_stack = rec->next;

    if (rec->type == IM_EDIT_INSERT) {
        im_buffer_delete_internal(buf, rec->pos, rec->len);
    } else {
        im_buffer_insert_internal(buf, rec->pos, rec->text, rec->len);
    }
...
im_result_t im_buffer_redo(im_buffer_t* buf) {
    if (!buf->redo_stack) return IM_ERR;
...
im_result_t im_buffer_batch_insert(im_buffer_t* buf, im_batch_insert_t* inserts, size_t count) {
    for (size_t i = 0; i < count; i++) {
        im_buffer_insert(buf, inserts[i].pos, inserts[i].text, inserts[i].len);
    }
    return IM_OK;
}
...
im_result_t im_buffer_batch_delete(im_buffer_t* buf, im_batch_delete_t* deletes, size_t count) {
    for (size_t i = 0; i < count; i++) {
        im_buffer_delete(buf, deletes[i].pos, deletes[i].len);
    }
    return IM_OK;
}
